...
from finops_pack.analysis.action_opportunities import build_action_opportunities
...
from finops_pack.analysis.schedule_recommendations import (
    ESTIMATED_STATUS,
...
from finops_pack.integrations.aws.collectors.ebs import collect_ebs_inventory
from finops_pack.integrations.aws.collectors.ec2 import collect_ec2_inventory
from finops_pack.integrations.aws.collectors.ecs import collect_ecs_inventory
from finops_pack.integrations.aws.collectors.lambda_functions import collect_lambda_inventory
from finops_pack.integrations.aws.collectors.nat_gateways import collect_nat_gateway_inventory
from finops_pack.integrations.aws.collectors.organizations import list_accounts
from finops_pack.integrations.aws.collectors.rds import collect_rds_inventory
from finops_pack.integrations.aws.collectors.s3 import collect_s3_inventory
from finops_pack.integrations.aws.cost_explorer import (
    ...
    build_resource_daily_request_kwargs,
...
from finops_pack.integrations.policy.iam_policy_generator import render_policy, write_policy
...
from finops_pack.orchestration.demo_fixtures import load_demo_fixture_bundle
...
from finops_pack.reporting.export_schema import write_export_recommendations_schema
...
def _check_resource_level_costs(session: Any) -> AccessCheck:
    ...
    try:
        ...
        response = client.get_cost_and_usage_with_resources(
            **build_resource_daily_request_kwargs(
                time_period=_recent_completed_day_window(),
            )
        )
    except ClientError as exc:
        code, message = _extract_client_error(exc)
    ...
    has_resource_rows = any(
        isinstance(result, dict)
        and isinstance(result.get("Groups"), list)
        and bool(result.get("Groups"))
    ...
    if not has_resource_rows:
        return AccessCheck(
            ...
            enabled=True,
            reason=(
                "Resource-level daily cost data is queryable, but no recent EC2 "
                "resource-level cost rows were returned."
            ),
            checked_in_region=BILLING_CONTROL_PLANE_REGION,
        )
    ...
    *,
    ...
    try:
        ...
        resource_daily_snapshot = {
            ...
            "request": build_resource_daily_request_kwargs(time_period=time_period),
            "pages": [],
            "resultsByTime": [],
            "timePeriodCount": 0,
            "groupCount": 0,
    ...
    *,
    ...
    try:
        summaries_snapshot = list_recommendation_summaries(
            session,
            region_name=region_name,
            group_by=DEFAULT_SUMMARY_GROUP_BY,
            rate_limit_safe_mode=rate_limit_safe_mode,
        )
    except RuntimeError as exc:
        summaries_snapshot = {
            "operation": "ListRecommendationSummaries",
            "request": {"groupBy": DEFAULT_SUMMARY_GROUP_BY},
            "pages": [],
            "items": [],
            "itemCount": 0,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    run_parser.add_argument(
        ...
        metavar="REGION",
        help=(
            "Optional scan regions for inventory collectors, for example "
