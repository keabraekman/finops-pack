...
from finops_pack.domain.levers import BUCKET_ORDER, LEVER_ORDER, lever_label
...
SOURCE_LABELS = {
    "AWS COH": "AWS Cost Optimization Hub",
    "AWS Compute Optimizer": "AWS Compute Optimizer",
    "CE fallback": "AWS billing signal",
    "Mixed / derived": "Combined analysis",
    "Native finops-pack": "Platform analysis",
}
CHECK_LABELS = {
    "cost_optimization_hub": "AWS recommendation coverage",
    "cost_explorer": "Cost Explorer baseline",
    "resource_level_costs": "Resource-level cost data",
}
MODULE_LABELS = {
    "cost_optimization_hub": "AWS recommendation coverage",
    "cost_explorer": "Cost Explorer baseline",
...
def _build_executive_summary(
    ...
    summary = f"Reviewed {len(account_map)} AWS account"
    if len(account_map) != 1:
        summary += "s"
    summary += "."
    if len(account_map) > 1:
        summary += (
            f" Environment split: {len(grouped['prod'])} prod, "
    ...
    if degraded_modules:
        summary += f" {len(degraded_modules)} data source(s) returned partial coverage."
    return summary
...
def _build_executive_summary_cards(
    ...
    *,
    ...
    if coh_context is not None and coh_total_display is not None:
        cards.append(
            {
                "label": "AWS recommendation savings",
    ...
    if schedule_context is not None:
        cards.append(
            {
                "label": "Modeled schedule savings",
...
def _build_prerequisites_context(access_report: AccessReport | None) -> dict[str, Any] | None:
    ...
    for check in access_report.checks:
        ...
        items.append(
            {
                "label": CHECK_LABELS.get(check.check_id, check.label),
                "status": check.status,
...
def _build_remediation_context(
    ...
    *,
    ...
    if coh_check is not None and (coh_check.enabled is not True or not has_coh_recommendations):
        add_step("Additional analysis coverage", OPTIONAL_CE_FALLBACK_NOTE)
...
def _build_collection_notes(access_report: AccessReport | None) -> list[dict[str, str]]:
    """Return degraded collector notes that should be visible on the main dashboard."""
    if access_report is None:
        return []

    return [
        {
            "label": MODULE_LABELS.get(module.module_id, module.label),
            "status": module.status,
            "reason": module.reason,
        }
        for module in access_report.modules
        if module.status == "DEGRADED" and module.reason
    ]
...
def _build_savings_by_environment(
    ...
    *,
    ...
    environment_totals: dict[str, dict[str, str | int | float]] = {
        environment: {
            "environment": environment,
    ...
    for row in recommendation_rows:
        bucket = environment_totals[str(row["environment"])]
        bucket["opportunity_count"] = int(bucket["opportunity_count"]) + 1
        bucket["estimated_monthly_savings"] = (
            float(bucket["estimated_monthly_savings"]) + float(row["estimated_monthly_savings"])
        )

    total_savings = sum(
        float(item["estimated_monthly_savings"]) for item in environment_totals.values()
    )
    aggregated_rows = [
    ...
    if (
        int(aggregated_rows[-1]["opportunity_count"]) == 0
        and float(aggregated_rows[-1]["estimated_monthly_savings"]) == 0
    ):
        aggregated_rows = aggregated_rows[:-1]

    for row in aggregated_rows:
        row["monthly_savings_display"] = _format_currency(
            float(row["estimated_monthly_savings"]),
            currency_code,
        )
        row["share_display"] = (
            f"{(float(row['estimated_monthly_savings']) / total_savings) * 100:.1f}%"
            if total_savings > 0
            else "0.0%"
        )
    return aggregated_rows
...
def _display_source_label(source_label: str) -> str:
    """Return a customer-facing source label."""
    return SOURCE_LABELS.get(source_label, source_label)
...
def _source_trust_note(action: ActionOpportunity) -> str:
    ...
    if action.source_label == "CE fallback":
        return "Derived from AWS billing signals"
    if action.source_label == "Mixed / derived":
        return "Derived from multiple AWS signals"
    if _is_modeled_schedule_action(action):
        return "Modeled from recent off-hours usage"
    return "Derived from inventory and recent AWS metrics"
        ...
        rows.append(
            {
                ...
                "source_label": _display_source_label(action.source_label),
...
def _build_action_context(
    ...
    *,
    ...
    default_trust_note = (
        "AWS recommendations come directly from AWS recommendation services. "
        "Schedule savings are modeled from recent off-hours usage patterns. "
        "Platform analysis applies conservative heuristics where AWS does not provide "
        "a direct recommendation."
    )
    empty_state_note = "No owner-facing actions were generated for this run yet."
    if total_monthly_savings > 0:
        if aws_verified_monthly_savings > 0 and modeled_schedule_monthly_savings > 0:
        ...
        elif aws_verified_monthly_savings > 0 and additional_native_monthly_savings == 0:
            hero_headline = (
                "Estimated monthly savings: "
                f"{_format_currency(aws_verified_monthly_savings, 'USD')}/mo in AWS-generated "
                "savings opportunities."
            )
        elif modeled_schedule_monthly_savings > 0 and additional_native_monthly_savings == 0:
            hero_headline = (
                "Estimated monthly savings: "
                f"{_format_currency(modeled_schedule_monthly_savings, 'USD')}/mo in modeled "
            ...
            hero_headline = (
                ...
                f"{_format_currency(total_monthly_savings, 'USD')}/mo in ranked savings "
                "opportunities."
            )
        trust_note = default_trust_note
    elif collection_note_list:
        hero_headline = "No ranked savings actions were produced in this run."
        trust_note = (
            "Some data sources returned partial coverage during this run. Review the coverage "
            "notes before treating this as a clean zero-result."
        )
        empty_state_note = (
            "No savings actions were generated. Review the coverage notes below for "
            "partial data collection."
        )
    else:
        hero_headline = "No ranked savings actions were identified for the current review scope."
        trust_note = (
            "The available AWS billing and inventory signals did not produce actionable "
            "savings findings for this run."
        )
        empty_state_note = "No savings actions were generated for the current review scope."
    ...
    return {
        ...
        "collection_notes": collection_note_list,
...
def _build_dashboard_template_context(
    ...
    *,
    ...
    collection_notes = _build_collection_notes(access_report)
...
def render_dashboard_html(
    account_map: list[AccountMapEntry],
    *,
    title: str = "AWS Savings Review",
    subtitle: str = (
        "Estimated AWS savings opportunities, prioritized actions, and "
        "supporting evidence."
    ),
...
def render_appendix_html(
    ...
    *,
    title: str = "AWS Savings Review",
    subtitle: str = (
        "Supporting evidence, validation details, and downloadable files for "
        "this review."
    ),
    report_mode: str = "lead_magnet",
    stylesheet_path: str | None = None,
...
def write_dashboard(
    ...
    *,
    ...
    copied_stylesheet_path = destination_path.parent / "style.css"
    shutil.copyfile(STATIC_DIR / "style.css", copied_stylesheet_path)
