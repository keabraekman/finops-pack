...
def merge_run_config(
    ...
    *,
    ...
    cli_regions = _normalize_region_list(regions) if regions is not None else None
    if region is not None:
        merged_region = region
    elif cli_regions:
        merged_region = cli_regions[0]
    else:
        merged_region = file_config.region

    if cli_regions is not None:
        merged_regions = _merge_regions(merged_region, cli_regions)
    else:
