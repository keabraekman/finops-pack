...
The narrowest built-in policy for current default behavior is `iam/policy-min.json`.
...
`iam/policy-full.json` adds optional capabilities that are disabled by default in the CLI:
...
bucket policy lives in [`docs/private-report-bucket-policy.json`](/Users/keabraekman/Documents/finops-pack/docs/private-report-bucket-policy.json).
